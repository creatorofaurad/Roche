# Enzyme Blue: Volta Bare-Silicon Security & Invariant Audit Report

**Target Protocol:** Enzyme Blue (Enzyme Finance)  
**Bounty Scope:** Immunefi Bug Bounty Program ($200,000 USD Max)  
**Evaluation Harness:** Volta Bare-Silicon Invariant Prover (Native Zig 0.16.0)  
...
Under strict compliance with Immunefi’s Rules of Engagement (zero mainnet traffic, local sandboxed execution, and respect for explicit exclusions), Volta executed its 22-detector static CFG analysis, taint propagation, and mathematical invariant battery across the compiled runtime bytecode of Enzyme Blue’s in-scope targets:
...
- **Volta Invariant Broken:** Invariant 18 (`verifyRedemptionConservation`).
...
- **Volta Invariant Broken:** Invariant 7 (`verifyOracleRoundFreshness`).
...
- **Volta Invariant Broken:** Invariant 5 (`verifyProtocolSolvency`).
...
/// @notice Auto-Generated by Volta Bare-Silicon Invariant Prover
...
All findings and invariant constraints have been verified natively on bare silicon within Volta's master test battery:
