//! tests/live_protocol_tests.zig: Live Real-World Protocol Attack Suite for Volta
//! Pure Zig 0.16.0 with 0 Dynamic Heap Allocations.

const std = @import("std");
const volta = @import("../src/main.zig");
const types = volta.types;
const storage = volta.storage;
const fuzzer = volta.fuzzer;
const cfg = volta.cfg;
const detectors = volta.detectors;
const invariants = volta.invariants;
const vm = volta.vm;
const arena = volta.arena;
...
test "Live Target 1: Euler V2 Vault Donation & Reentrancy Vulnerability Detection" {
    var engine = volta.VoltaEngine.init();
